...
#### Install the geoip database and maxmind tools

Install the maxmind [system library](https://github.com/maxmind/libmaxminddb#on-ubuntu-via-ppa):
...
Disable the modules we don't want:
...
Unfortunately we need to recompile nginx to use the `geoip2` module, which is rather inconvenient, but not difficult to do.

Install required build packages:

```bash
apt-get install -y libpcre3-dev libssl-dev zlib1g-dev libxslt1-dev libgd-dev
```

Download the `geoip2` module:

```bash
git clone https://github.com/leev/ngx_http_geoip2_module.git /root/ngx_http_geoip2_module
```

Download the matching nginx version:
...
Get the current nginx version and `configure arguments`:

```bash
nginx -V
```

Configure nginx using the `configure arguments`:

```bash
./configure CONFIGURE_ARGUMENTS --add-dynamic-module=/root/ngx_http_geoip2_module
```
...
```bash
echo "load_module modules/ngx_http_geoip2_module.so;" > /etc/nginx/modules-enabled/50-mod-http-geoip2.conf
```
...
<!-- # Move the module to the nginx modules folder
...
See: https://medium.com/@maxime.durand.54/add-the-geoip2-module-to-nginx-f0b56e015763
